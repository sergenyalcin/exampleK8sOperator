...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "60785ef2.example",
	})
	if err != nil {
	...
	if err = (&controllers.SourceReconciler{
		...
		Log:    ctrl.Log.WithName("controllers").WithName(examplev1.SourceKind),
	...
	if err = (&controllers.DestinationReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrl.Log.WithName("controllers").WithName(examplev1.DestinationKind),
	}).SetupWithManager(mgr); err != nil {
